...
- **A version section calls for `just test`.** A release ships every command, so the whole live suite is what confirms one, and it is the user's to run. Ask for it as part of writing the section, and say the release is unconfirmed until it passes - `just test-fast` cannot answer for a release.
...
- **Name the tests, as one command, and let it record.** `just coverage-one` takes a regex, so the whole ask is one line: `just coverage-one 'TestContactsCRUD|TestContactsExportAndImportRoundTrip'`. It is `just test-one` plus the recording, so it is what to ask for by default: a change that reached a new endpoint leaves it in `tests/api-coverage.golden`, and one that reached nothing new leaves the file alone. Beside the command, one line per test saying what that test proves about the change, so the user can judge the list rather than trust it. Read `tests/live/` to find them; a test that does not exist yet is a test to write first, not a reason to run everything.
...
- **Ask for `just test` only when a subset cannot answer** - a change under every command (`kit.Run`, the client, the key hierarchy, the harness itself), or a release, where the version section in `CHANGELOG.md` ships all of them. Then say which. "To be safe" is not one of them.
...
6. **Coverage, when the CLI's requests moved** - `just test-fast` fails when the CLI can send a request no test has ever sent, which is how an agent finds out. It is recorded by a live run, so it is the user's: **`just coverage-one PATTERN` is the ask** - the same subset the change wanted confirmed, adding what it reached to `tests/api-coverage.golden`. `just coverage` runs everything and rewrites the file, which is what it takes to *remove* a line a subset cannot know is stale. Either way the diff is committed, so it is read together. See [Testing](#testing).
...
- **`just coverage-one PATTERN` is how a change is confirmed live**, and what to name by default. It takes a regex, so one command covers every test a change touches, and it records what those tests reached on top of what was recorded before. `just test-one PATTERN` is the same run without touching the recording; the whole suite is for changes under every command and for a release, and nothing else. What to suggest after implementing is spelled out under [Quality Gates](#quality-gates).

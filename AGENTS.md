...
- **Ask for `just test` only when a subset cannot answer** - a change under every command (`kit.Run`, the client, the key hierarchy, the harness itself). Then say which. "To be safe" is not one of them.

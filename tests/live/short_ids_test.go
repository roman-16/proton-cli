...
// Short IDs: what a listing prints, and what typing it back finds again.
//
// A listing shows eight characters to a terminal and the whole thing to a pipe,
// because one is for reading and the other for a script. The cache is what turns
// the short form back into the full one, and it is read as it lands on disk.
...
func TestShortIDJSONAlwaysFull(t *testing.T) {
	...
	if len(msgs) == 0 {
		t.Fatal("the fixture guarantees a delivered message and the listing is empty")
	}
...
func idcachePath(t *testing.T) string {
	...
	return filepath.Join(cd, "proton-cli", "idcache", account.Primary+".json")
}

...
- **A listing carries no secret.** Only `items get`, `revisions get`, `links get` and `pass export` decrypt secret fields, and `access-tokens create` is the one moment a token is in the clear; a list row is a type that cannot hold one.

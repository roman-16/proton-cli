...
- **One rewrite of a key list is allowed, and it is one bit wide.** Turning an address's end-to-end encryption on or off is published by its key list, so `keys.SetEncryption` derives the new `Data` from the list Proton serves by flipping `FLAG_EMAIL_NO_ENCRYPT` on every entry - no key added, removed or reordered, and the served list checked against the keys the account holds first. Only forwarding to an address outside Proton reaches it, in either direction: setting one up turns encryption off, and deleting the last one turns it back on.

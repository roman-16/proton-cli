...
- **One flag name, one meaning.** `--to` is a mail recipient, `--into` a container on Proton's side, `--dest` a local path, `--force` overwriting a local file, `--all` everything in scope. A new shared flag is declared in `kit.Flags`, in `internal/cli/kit/flags.go`. Only the root owns single-letter forms, and there are five: `-p`, `-o`, `-n`, `-q`, `-y`.
- **A password is never a flag value.** `argv` is readable through `ps` and survives in shell history and unit files, so a secret comes from `--password-file` or `--secret-file NAME=FILE`, either of which reads standard input when the path is `-`. A password chosen for somebody else gets its own name (`--link-password-file`, `--eo-password-file`) and is bounded here, not by the server.
...
- **A page size is the user's, not Proton's.** `--limit N` yields N rows whatever the endpoint serves per page, `0` yields everything, and the same word caps a bulk verb. A collection Proton hands over whole answers whole unless a page was asked for: the rows are already here, so cutting them uninvited discards what the request fetched.
...
- **A name out of the account reaches an error only through `errs.Naming`.** Every other value a message picks up has a shape - an address, an ID, a path - and `internal/redact` finds it again wherever the message ends up. What somebody called a vault item or a file has none: it is words, and nothing can pick it back out of a finished sentence. So the name travels beside the error rather than only inside it, which is what lets the reader be told which of their things failed while the log `proton report` hands over says `<name>`. A message written for a person is free to name an address, because the file that shares it rewrites the sentence on its way out and the screen does not.
- **A listing carries no secret.** Only `items get`, `revisions get`, `links get` and `pass export` decrypt secret fields, and `access-tokens create` and `smtp-tokens create` are the two moments a token is in the clear; a list row is a type that cannot hold one.
- **An address's key list changes only the way Proton's clients change it.** Every change to an address key - generating one, importing one, bringing a locked one back from a copy, making one primary, marking one obsolete or compromised, deleting one - is published with the list Proton serves edited the one way its clients edit it for that change: a key named first as the new primary or last as one that reads, the primary moved, one entry's flags set, or one entry dropped. Turning end-to-end encryption on or off flips `FLAG_EMAIL_NO_ENCRYPT` on every entry, and a key a forwarder derived is published with the list re-signed unchanged, since a list never names one. The served list is checked against the keys the account holds before anything is edited, a list is composed only for an address with no key in use, and it is signed by the keys that are primary once the change lands. A key made or imported is read back before the command reports success.
- **A credential proton writes leaves as a verifier, and a key it re-locks is the key that was there.** Four are written: the Pass extra password, the account password, the second password, and the bytes behind a recovery phrase. Each goes to Proton as an SRP verifier and never as itself. Two of them also re-lock the user keys - `keys.Relock` for a password, `keys.NewRecoveryPhrase` for a second copy under the phrase - and neither touches the key material, the address keys, whose tokens no password enters, or a key a reset left shut. An account whose address keys predate that move, and an administrator whose organization key is locked with their password, are refused rather than left half-moved: both have something a change here would leave behind, and neither failure is one anybody can put right.
- **Every key this build writes has one shape**, `keys.Generation` and `keys.LockAndArmor`: an EdDSA primary over Curve25519, SHA-512 signatures, AES-256 and SHA-512 preferences, armour with no headers, and Proton's clock rather than the machine's. Proton refuses keys that differ from what its own clients write over choices the standard calls equivalent, so a new key goes through them rather than through a library's defaults.

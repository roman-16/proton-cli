...
The stored card goes out whole, so properties this tool has no flag for are exported too.
...
Nothing is merged. Importing the same file twice creates duplicates. Use `merge` afterwards to fold them together.
